...
#define MetaOf(type) (typename std::decay_t<type>::REFL_META_INFO_T_NAME())

#define GetMemberMeta(type, member)                                             \
    ([&]() constexpr {                                                          \
        constexpr size_t id = type::REFL_META_INFO_T_NAME::get_meta_id(member); \
...
namespace __reflitest {
    struct reflitest_impl {
        ...
        static M get_member_type(M T::*) { return *reinterpret_cast<M *>(nullptr); }
    };
}  // namespace __reflitest
